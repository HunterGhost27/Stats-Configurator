...
- Alternatively, if you use **LaughingLeader's Mod-Manager**, you can go to *Settings > Preferences > Script-Extender*, check `Enable Console Window`and then `Export Settings`.
...
- Users create their configs in `json` files. These config-files can be loaded-in at any time by the **host-character** using the mod-menu's `Load Configuration` option. Some stat-edits (like skills) are applied instantaneously but most (like characters) will require a _save-and-reload_.
- ===
...
- ===
- Applying configurations this way is great for **prototyping** or making **temporary changes**. But instead of re-applying configurations every session, you should export the configs to **ConfigData**. This will allow the StatsLoader(Read more below) to apply those edits automatically when the game loads.

### StatsLoader

- The **StatsLoader** is responsible for loading **ConfigData** when the module loads. Stat-modifications applied through the StatsLoader do not require additional synchronization.
...
- By prefixing an stat-attribute with special characters you can modify how those changes are applied. This allows you to make relative changes instead of absolute ones. For example,
- `"Shout_InspireStart"`: {
-   `"-Cooldown"`: 1,
-   `"+ActionPoints"`: 2
- }
...
- Treasure Category
- ===
- Checkout the **extensive-documentation** for more details.
